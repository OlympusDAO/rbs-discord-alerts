...
- `discordWebhookEmissions`: CD auction tuning and auction results
...
```bash
export EMISSIONS_WEBHOOK_URL="..."
export PROTOCOL_BUYBACKS_WEBHOOK_URL="..."
pulumi config set --secret discordWebhookEmissions "$EMISSIONS_WEBHOOK_URL" --stack dev
pulumi config set --secret discordWebhookProtocolBuybacks "$PROTOCOL_BUYBACKS_WEBHOOK_URL" --stack dev
pulumi config set --secret discordWebhookEmissions "$EMISSIONS_WEBHOOK_URL" --stack prod
